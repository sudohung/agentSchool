...
class APIConfig:
    ...
    DATABASE_PATH is loaded from embedder_config.json's `knowledge_db_path` if present,
    otherwise falls back to the KNOWLEDGE_DB_PATH env var or a default file.
    """
    MAX_LIMIT: int = 1000  # Maximum number of records to return in a single request

    @staticmethod
    def _load_from_embedder_config() -> str:
        cfg_path = os.path.join(os.path.dirname(__file__), "embedder_config.json")
        try:
    ...
    DATABASE_PATH: str = _load_from_embedder_config()
...
class MemoryAPIClient:
    ...
    def __init__(self, db_path: str):
        ...
        if parent and not os.path.exists(parent):
            try:
                ...
                logger.warning(f"Could not create directory for database path: {parent}")
    ...
    def get_connection(self):
        ...
        conn = None
        try:
            # Use check_same_thread=False to allow connections from different threads (uvicorn workers)
    ...
    def _initialize_database(self):
        """Create necessary tables and pragmas for the knowledge database."""
        # Connect directly for initialization (use same flags as runtime)
        conn = sqlite3.connect(self.db_path, check_same_thread=False)
        try:
            cur = conn.cursor()
            # Improve concurrency and durability settings
            ...
            cur.execute(
                """
                CREATE TRIGGER IF NOT EXISTS trg_knowledge_entries_updated_at
                AFTER UPDATE ON knowledge_entries
                FOR EACH ROW
                BEGIN
                    UPDATE knowledge_entries SET updated_at = datetime('now') WHERE id = OLD.id;
                END;
                """
            )

            # Try to create an FTS5 virtual table for full-text search and helper triggers.
            try:
                cur.execute(
                    "CREATE VIRTUAL TABLE IF NOT EXISTS knowledge_entries_fts USING fts5(title, content, content='knowledge_entries', content_rowid='id')"
                )

                # When using external content FTS5, we need to keep it in sync. Create triggers.
                cur.execute(
                    """
                    CREATE TRIGGER IF NOT EXISTS knowledge_entries_ai AFTER INSERT ON knowledge_entries BEGIN
                      INSERT INTO knowledge_entries_fts(rowid, title, content) VALUES (new.id, new.title, new.content);
                    END;
                    """
                )
                cur.execute(
                    ...
                    CREATE TRIGGER IF NOT EXISTS knowledge_entries_ad AFTER DELETE ON knowledge_entries BEGIN
                      INSERT INTO knowledge_entries_fts(knowledge_entries_fts, rowid, title, content) VALUES('delete', old.id, old.title, old.content);
                    END;
                    """
                )
                cur.execute(
                ...
                # FTS5 may not be available in some sqlite builds; fall back silently.
                logger.info("SQLite FTS5 not available; full-text search will use LIKE queries.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8425,
        help="Port to listen on (default: 8425)"
    )
    parser.add_argument(
        "--reload",
        action="store_true",
